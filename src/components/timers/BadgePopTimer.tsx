...
export const BadgePopTimer: React.FC<DesignerTimerProps> = (props) => {
  ...
  return (
    ...
    >
      ...
      <text x={state.width / 2} y={state.height * 0.16} textAnchor="middle" fontSize={state.height * 0.16} fontWeight={900} fill={palette.shell} letterSpacing="0.2em" fontFamily='"Segoe UI", Arial, sans-serif'>
        TIMER
      </text>
      <text x={state.width / 2} y={state.height * 0.66} textAnchor="middle" fontSize={state.height * 0.4} fontWeight={900} fill={palette.text} fontFamily='"Arial Black", "Segoe UI", sans-serif'>
        {state.secondsLabel}
      </text>
    </svg>
  );
